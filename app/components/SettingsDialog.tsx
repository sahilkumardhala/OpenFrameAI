...
import { PROVIDERS, modelSettings } from '../lib/settings'
import { SYSTEM_PROMPT } from '../prompt'

export function SettingsDialog({ onClose }: TLUiDialogProps) {
	const settings = useValue('settings', () => modelSettings.get(), [])

	useReactor(
		'update settings local storage',
		() => {
			localStorage.setItem('makereal_settings_2', JSON.stringify(modelSettings.get()))
		},
		[]
	)
	...
	return (
		<>
			...
			>
				<p>
					To use OpenFrameAI, enter your API key for each model provider that you wish to use. Draw
					some shapes, then select the shapes and click Start.{' '}
					<a
						target="_blank"
				...
				<div style={{ display: 'flex', flexDirection: 'column', gap: 4, marginTop: 8 }}>
					...
					<select
						...
						onChange={(e) => {
							modelSettings.update((s) => ({ ...s, provider: e.target.value as any }))
						}}
					>
				...
				<div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
					...
					<TldrawUiInput
						...
						onValueChange={(value) => {
							modelSettings.update((s) => ({ ...s, prompts: { ...s.prompts, system: value } }))
						}}
					/>
				</div>
			</TldrawUiDialogBody>
	...
	return (
		<div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
			...
			<TldrawUiInput
				...
				onValueChange={(value) => {
					modelSettings.update((s) => ({ ...s, keys: { ...s.keys, [provider.id]: value } }))
				}}
			/>
		</div>
	)
